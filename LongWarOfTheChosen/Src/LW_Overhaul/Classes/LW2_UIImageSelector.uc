...
class LW2_UIImageSelector extends UIPanel;
...
var array<LW2_UISquadImage> ImageChips; 
...
simulated function LW2_UIImageSelector InitImageSelector(optional name InitName, 
...
{
	local LW2_UISquadImage Chip, LeftNavTargetChip, RightNavTargetChip, UpNavTargetChip, DownNavTargetChip; 
	local int iChip, iRow, iCol, iMaxChipsPerRow, iLastRowLength, iOffset; 
	
	Images = _Images;

	iMaxChipsPerRow = int( (width - ITEM_PADDING) / ( class'LW2_UISquadImage'.default.iDefaultSize + ITEM_PADDING ) );
	...
	for( iChip = 0; iChip < Images.Length; iChip++ )
	{
		...
		Chip = Spawn( class'LW2_UISquadImage', ChipContainer);
		Chip.InitSquadImage( '',
							iChip, 
							Images[iChip], 
							GetChipX(iCol), 
							GetChipY(iRow), 
							class'LW2_UISquadImage'.default.iDefaultSize, 
							iRow, 
							iCol,
							OnPreviewColor, 
							OnAcceptColor);
	...
	ChipHeight = GetChipY(iRow) + class'LW2_UISquadImage'.default.iDefaultSize; 
...
simulated function SetInitialSelection()
{
	...
	ScrollToRow(LW2_UISquadImage(ChipContainer.Navigator.GetSelected()).Row);
}

simulated function float GetChipX( int iCol )
{
	return ((class'LW2_UISquadImage'.default.iDefaultSize + ITEM_PADDING) * iCol) + (EDGE_PADDING); 
}

simulated function float GetChipY( int iRow )
{
	return ((class'LW2_UISquadImage'.default.iDefaultSize + ITEM_PADDING) * iRow) + (EDGE_PADDING); 
}
