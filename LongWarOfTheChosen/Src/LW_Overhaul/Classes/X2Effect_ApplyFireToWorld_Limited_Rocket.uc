//---------------------------------------------------------------------------------------
//  FILE:    X2Effect_ApplyFireToWorld_Limited_Rocket.uc
//  AUTHOR:  Amineri / Pavonis Interactive / Tedster
//  PURPOSE: Similar to X2Effect_ApplyFireToWorld, except fire damage is applied to the world over time, instead of immediately / Tedster - special one for rockets to limit to Concussion Warheads
//---------------------------------------------------------------------------------------

class X2Effect_ApplyFireToWorld_Limited_Rocket extends X2Effect_ApplyFireToWorld;
...
static simulated function SharedApplyFireToTiles( Name EffectName, X2Effect_ApplyFireToWorld FireEffect, XComGameState NewGameState, array<TilePosPair> OutTiles, XComGameState_Unit SourceStateObject, int ForceIntensity = -1, optional bool UseFireChance = false )
{
    ...
    // Tedster - hardcode ability req here
    If(!SourceStateObject.HasAbilityFromAnySource('ImprovedMunitions_LW'))
        return;
