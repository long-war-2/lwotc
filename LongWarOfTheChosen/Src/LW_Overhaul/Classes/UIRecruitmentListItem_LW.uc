...
//  AUTHOR:	 KDM
//  PURPOSE: A slightly modified version of the original UIRecruitmentListItem_LW, making the 
//	recruit list items compatible with Long War of the Chosen.
//--------------------------------------------------------------------------------------- 

class UIRecruitmentListItem_LW extends UIRecruitmentListItem config(LW_UI);
...
function InitIconValuePair(XComGameState_Unit Recruit, ECharStatType StatType, string MCRoot, string ImagePath, float XLoc, float YLoc)
{
	...
	if (StatValue == none)
	{
		...
		StatValue.InitText(name(MCRoot $ "_LW"));
...
function UpdateText(string MCRoot)
{
	...
	StatValue = UIText(GetChildByName(name(MCRoot $ "_LW"), false));
