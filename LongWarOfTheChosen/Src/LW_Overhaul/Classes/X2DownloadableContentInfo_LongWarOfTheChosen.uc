	...
	if (PodSize > 1)
	{
		...
		if (!swap)
		{
			for (k = 1; k < SpawnInfo.SelectedCharacterTemplateNames.Length; k++)
			{
				...
				if(default.bNerfFrostLegion && InStr("FROST", caps(SpawnInfo.SelectedCharacterTemplateNames[k]))!= INDEX_NONE)
				{
		...
		if (swap)
		{
			...
			While (!Satisfactory && Tries < 12)
			{
				...
				foreach SpawnInfo.SelectedCharacterTemplateNames(CharacterTemplateName, idx)
				{
					...
					if(default.bNerfFrostLegion && InStr("FROST", caps(SpawnInfo.SelectedCharacterTemplateNames[idx])) != INDEX_NONE)
					{
